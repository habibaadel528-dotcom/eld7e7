...
function AdminNavItem({ item, isCollapsed, onClick }) {
  return (
    <NavLink
      to={item.to}
      ...
      onClick={onClick}
      className={({ isActive }) =>
        `group relative flex items-center transition-all duration-150 text-start ${
          isCollapsed
      ...
      {item.badge ? (
        <span
          className={`flex h-5 min-w-[20px] items-center justify-center rounded-full bg-[#c53938] px-1 text-[10px] font-bold text-white ${
            isCollapsed ? 'absolute ltr:-right-1 rtl:-left-1 -top-1' : ''
          }`}
        >
          {item.badge}
        </span>
...
export default function AdminSidebar({ isCollapsed, onToggleCollapse, onNavClick, isMobile }) {
  const { lang, t } = useLanguage();
  const tr = t('admin').sidebar;

  const mainNav = [
    { to: '/admin/orders', label: tr.orders, icon: 'orders', badge: 2 },
    { to: '/admin/products', label: tr.products, icon: 'box' },
    { to: '/admin/customers', label: tr.customers, icon: 'users' },
  ];

  const secondaryNav = [
    { to: '/admin/settings', label: tr.settings, icon: 'settings' },
  ];
    ...
    >
      ...
      {!isMobile && (
        <div className={`flex items-center w-full mb-3 ${isCollapsed ? 'justify-center' : 'justify-between px-1'}`}>
          {!isCollapsed && (
            <span className="text-[11px] font-bold uppercase tracking-wider text-[var(--secondary-text)] opacity-70 text-start">
              {tr.menuTitle}
            </span>
          )}
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--surface-soft)] text-[var(--secondary-text)] transition-all hover:bg-[#c53938] hover:text-white hover:border-[#c53938] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c53938] cursor-pointer"
            title={isCollapsed ? tr.expand : tr.collapse}
      ...
      <nav aria-label="Admin navigation" className={`mt-2 flex flex-1 flex-col gap-1 w-full ${isCollapsed ? 'items-center' : ''}`}>
        {mainNav.map((item) => (
          <AdminNavItem key={item.to} item={item} isCollapsed={isCollapsed} onClick={onNavClick} />
        ))}
      ...
      <div className={`mt-auto border-t border-[var(--border-color)] pt-3 w-full ${isCollapsed ? 'hidden' : ''}`}>
        <div className="rounded-xl bg-[var(--surface-soft)] p-3 text-start">
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-emerald-500 shrink-0" />
            <span className="text-xs font-semibold text-[var(--primary-text)]">{tr.storeOnline}</span>
          </div>
          <p className="mt-1.5 text-[11px] text-[var(--secondary-text)]">
            {tr.storeStats('284', '3,420')}
          </p>
          <p className="text-[11px] text-[var(--secondary-text)]">{tr.newOrdersToday(2)}</p>
        </div>
      </div>
    </aside>
  );
